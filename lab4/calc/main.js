...
function evaluate(str) {
    let arrayOfAll = str.split(" ");
    let workingArray = [];
    let k = 0;
    for(let i in arrayOfAll){
        let workinkSymb = arrayOfAll[i];
        if(!isOperation(workinkSymb)){
            workingArray.push(workinkSymb);
        }
        else if(workinkSymb == '+'){
            let a = Number(workingArray.pop());
            let b = Number(workingArray.pop());
            workingArray.push(b+a);
        }
        else if(workinkSymb == '*'){
            let a = Number(workingArray.pop());
            let b = Number(workingArray.pop());
    ...
    if(workingArray[0] != Math.round(workingArray[0])) return Number(workingArray[0]).toFixed(2);
...
function clickHandler(event) {
    if(document.querySelector('.screen').getElementsByTagName('span')[0].innerHTML == 'NaN'){
        document.querySelector('.screen').getElementsByTagName('span')[0].innerHTML = '';
    }
    if(event.target.innerHTML == 'C'){
        screen = document.querySelector('.screen').getElementsByTagName('span')[0];
        screen.innerHTML = '';
    }
    if(event.target.innerHTML == '='){
        screen = document.querySelector('.screen').getElementsByTagName('span')[0];
        let screenStr = screen.innerHTML;
        let compiledScreen = compile(screenStr);
        let evaluatedScreen = evaluate(compiledScreen);
        screen.innerHTML = evaluatedScreen;
    }
    if(!((event.target.innerHTML).includes('button') || (event.target.innerHTML == 'C') || (event.target.innerHTML == '='))){
        screen = document.querySelector('.screen').getElementsByTagName('span')[0];
